...
elif [ "$WAIT_FOR_POSTGRES" = "1" ] && [ -z "${DATABASE_URL:-}" ]; then
  log "ERROR: DATABASE_URL is required (set it in .env)"
  exit 1
fi
...
if [ "$RUN_MIGRATIONS" = "1" ] && [ -n "${DATABASE_URL:-}" ]; then
  case "$DATABASE_URL" in
    *sqlite*)
      log "SQLite — skipping alembic (create_all handles test DBs)"
      ;;
    *)
      if command -v alembic >/dev/null 2>&1; then
        ...
        if ! run_alembic alembic upgrade head; then
          # Existing DB created via create_all before Alembic — stamp current head
          log "WARN: alembic upgrade failed; attempting stamp head for pre-existing schema"
          run_alembic alembic stamp head || log "WARN: alembic stamp failed; create_all will run on app start"
        fi
      else
        log "WARN: alembic not installed; app will use create_all"
      fi
      ;;
  esac
fi
